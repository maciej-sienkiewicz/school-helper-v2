...
import {
  ...
  mockNotes, mockRecordings, formatDuration,
} from '../../data/mockData';
...
function AddMaterialModal({
  ...
  const existingNotes = mockNotes.filter(
    n => n.topicId === topicId && n.status !== 'deleted'
  );
  const existingRecordings = mockRecordings.filter(
    r => r.topicId === topicId && r.classId !== currentClassId
  );
    ...
    >
      ...
      >
        ...
        <div className="flex gap-1 mb-4 p-1 bg-gray-100 rounded-2xl">
          <button
            onClick={() => { setTab('existing'); setSelectedFile(null); }}
            className={`flex-1 flex items-center justify-center gap-1.5 py-1.5 px-3 rounded-xl text-sm font-medium transition-all cursor-pointer ${
              tab === 'existing' ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-500 hover:text-gray-700'
            }`}
          >
          ...
          <button
            onClick={() => { setTab('file'); setSelectedId(null); }}
            className={`flex-1 flex items-center justify-center gap-1.5 py-1.5 px-3 rounded-xl text-sm font-medium transition-all cursor-pointer ${
              ...
              {items.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-gray-400">
                    Brak dostępnych {type === 'note' ? 'notatek' : 'nagrań'} dla tego tematu z innych klas.
                  </p>
                  <p className="text-xs text-gray-300 mt-1">
                    Możesz dodać {type === 'note' ? 'notatkę' : 'nagranie'} z pliku.
                  </p>
                </div>
              ) : (
                <div className="space-y-2">
                  {type === 'note' && existingNotes.map(note => {
                    const classNames = note.sharedWithClasses
                      .map(cid => mockClasses.find(c => c.id === cid)?.name ?? cid)
                      .join(', ');
                    return (
                      <button
                        key={note.id}
                      ...
                      >
                        <div className="flex items-start gap-2">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-gray-500 mt-0.5 line-clamp-2">{note.summary}</p>
                            {classNames && (
                              <div className="flex items-center gap-1 mt-1.5">
                                <Users className="w-3 h-3 text-gray-400" />
                                <span className="text-xs text-gray-400">{classNames}</span>
                              </div>
                            )}
                          </div>
                          {selectedId === note.id && (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <div
                          ...
                          style={{ background: rec.thumbnailColor ?? '#fde68a' }}
                        >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-xs text-gray-500">
                            {new Date(rec.date).toLocaleDateString('pl-PL')} · {formatDuration(rec.durationSeconds)}
                          </div>
                        </div>
                        {selectedId === rec.id && (
                          ...
                          >
                            <div className="divide-y divide-gray-50">
                              {topics.map(topic => {
                                ...
                                const missingNote = !status?.hasNote;
                                const missingRecording = !status?.hasRecording;

                                return (
                                  <div
                                    key={topic.id}
                                    className="group/row flex items-center gap-3 px-4 py-3 hover:bg-violet-50/50 transition-colors"
                                  >
                                    <button
                                    ...
                                    <div className="flex items-center gap-1.5 flex-shrink-0">
                                      {/* Note: show icon if has note, else show add button */}
                                      {status?.hasNote ? (
                                        ...
                                        <button
                                          ...
                                          className="w-5 h-5 rounded-lg border border-dashed border-violet-200 flex items-center justify-center text-violet-300 hover:border-violet-400 hover:text-violet-500 hover:bg-violet-50 transition-all cursor-pointer opacity-0 group-hover/row:opacity-100"
                                        >
                                      ...
                                      {/* Recording: show icon if has recording, else show add button */}
                                      {status?.hasRecording ? (
                                        ...
                                        <button
                                          title="Dodaj nagranie"
                                          onClick={() => setAddMaterialModal({
                                            type: 'recording',
                                            topicId: topic.id,
                                            topicName: topic.name,
                                          })}
                                          className="w-5 h-5 rounded-lg border border-dashed border-amber-200 flex items-center justify-center text-amber-300 hover:border-amber-400 hover:text-amber-500 hover:bg-amber-50 transition-all cursor-pointer opacity-0 group-hover/row:opacity-100"
                                        >
                                          <Plus className="w-3 h-3" />
                                        </button>
                                      )}
      ...
      <AnimatePresence>
        {addMaterialModal && (
          <AddMaterialModal
            ...
            onConfirm={() => {
              alert(`Dodano ${addMaterialModal.type === 'note' ? 'notatkę' : 'nagranie'} do tematu: ${addMaterialModal.topicName}`);
