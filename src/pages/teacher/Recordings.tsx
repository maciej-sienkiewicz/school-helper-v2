...
import { mockRecordings, mockNotes, mockSharedMaterials, formatDuration } from '../../data/mockData';
import type { Recording, Note, SharedMaterial } from '../../types';
      ...
      <div className="bg-white rounded-4xl shadow-2xl p-8 max-w-lg w-full" onClick={e => e.stopPropagation()}>
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <input
                type="range" min="0" max="100" value={end}
    ...
    >
      ...
      >
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <Button variant="primary" size="md" icon={<Share2 className="w-4 h-4" />} onClick={() => alert('Wybierz klasy do udostępnienia')}>
            Udostępnij
          </Button>
      ...
      >
        ...
        <div className="flex items-center gap-4 p-5" style={{ background: rec.thumbnailColor + '40' }}>
          <div
            ...
            style={{ background: rec.thumbnailColor }}
          >
        ...
        <div className="px-5 py-3 bg-white flex flex-wrap items-center gap-2 border-t border-gray-50">
          ...
          {(noteGenerated || rec.status === 'has_note') && (
            <Button
              ...
              onClick={() => alert('Wybierz klasy do udostępnienia')}
            >
              Udostępnij
            </Button>
          )}
...
function SharedMaterialRow({ mat }: { mat: SharedMaterial }) {
        ...
        {expanded && (
          ...
          >
            <div className="p-5 space-y-5">
              ...
              {/* Per-class stats */}
              <div>
                <div className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Statystyki klas</div>
                <div className="space-y-2">
                  {mat.stats.classStats.map(cs => {
                    ...
                    return (
                      <div key={cs.classId} className="p-3 bg-gray-50 rounded-2xl">
...
export function Recordings() {
  ...
  // Group shared materials by grade
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'shared' && (
            <motion.div
              ...
              className="space-y-8"
            >
              {Object.entries(byGrade).sort(([a], [b]) => Number(a) - Number(b)).map(([grade, mats]) => (
                <div key={grade}>
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="font-bold text-gray-800">
                      {Number(grade) <= 8 ? `Klasa ${grade} (Szkoła podstawowa)` : `Klasa ${Number(grade) - 8} (Liceum)`}
                    </h3>
                    <Badge variant="purple">{mats.length} materiałów</Badge>
                  </div>
                  <div className="space-y-3 pl-11">
                    {mats.map(mat => (
                      <SharedMaterialRow key={mat.id} mat={mat} />
                    ))}
                  </div>
                </div>
              ))}
