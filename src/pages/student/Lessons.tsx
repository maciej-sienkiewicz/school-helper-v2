...
export function StudentLessons() {
  ...
  const subjects = useMemo(
    () => [...new Set(mockStudentLessons.map(l => l.subject))],
    []
  );

  const filtered = useMemo(() => {
    const q = search.trim().toLowerCase();
    return mockStudentLessons.filter(l => {
      if (subjectFilter && l.subject !== subjectFilter) return false;
      if (onlyWithNotes && !l.noteId) return false;
      if (q && !l.topicName.toLowerCase().includes(q) && !l.unitName.toLowerCase().includes(q)) return false;
      return true;
    });
  }, [search, subjectFilter, onlyWithNotes]);

  // group by unitName for display
  const grouped = useMemo(() => {
    const map = new Map<string, StudentLesson[]>();
    for (const l of filtered) {
      const key = `${l.subject} › ${l.unitName}`;
    ...
    <div className="p-6 max-w-3xl mx-auto">
      ...
      <div className="space-y-3 mb-6">
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Szukaj tematu lub działu..."
            className="w-full pl-10 pr-4 py-2.5 bg-white border border-gray-200 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-sky-300 focus:border-transparent shadow-sm"
          />
          {search && (
            <button
              onClick={() => setSearch('')}
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-6">
          {[...grouped.entries()].map(([group, lessons]) => (
            <div key={group}>
              <div className="flex items-center gap-2 mb-2 px-1">
                <div className="h-px flex-1 bg-gray-200" />
                <span className="text-xs font-bold text-gray-400 uppercase tracking-wider whitespace-nowrap">{group}</span>
                <div className="h-px flex-1 bg-gray-200" />
              </div>
